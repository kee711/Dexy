...
export class DexyClient {
  ...
  constructor(options: DexyClientOptions) {
    if (!options?.apiKey) {
      throw new DexyError("apiKey is required", { code: "API_KEY_REQUIRED", status: 400 });
    }
  ...
  async run(options: RunOptions): Promise<ExecuteResult> {
    if (!options?.prompt) {
      throw new DexyError("prompt is required", { code: "PROMPT_REQUIRED", status: 400 });
    }

    this._log("run:start", { stream: options.stream });

    const search = await this.search({ prompt: options.prompt, meta: options.meta });
    this._log("run:search:ok", { agentId: search.agentId, confidence: search.confidence });
    ...
    this._log("run:execute:ok", { agentId: search.agentId, stream: Boolean(exec.stream) });
    return exec;
  }
  ...
  async execute(options: ExecuteOptions): Promise<ExecuteResult> {
    if (!options?.agentId) {
      throw new DexyError("agentId is required", { code: "AGENT_ID_REQUIRED", status: 400 });
    }
    ...
    if (options.stream) {
      ...
      return { output: "", usage: this._defaultUsage(), stream, agent: { id: options.agentId } };
    }
    ...
    let payload: ExecuteApiResponse | null = null;
    try {
    ...
    const usage = this._normalizeUsage(payload?.normalized?.usage ?? payload?.usage);

    return {
      output,
      usage,
      agent: payload?.agent ?? { id: options.agentId },
    };
  }
  ...
  private async _stream(path: string, body: unknown): Promise<AsyncIterable<string>> {
    let res: Response;
  ...
  // Minimal SSE parser that yields `data:` payloads one by one.
